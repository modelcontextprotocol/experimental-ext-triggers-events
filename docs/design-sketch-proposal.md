...
**Poll mode:** The SDK runs a background loop per subscription that calls `events/poll`, passing a `waitMs` for the server to hold the request open, and waits `nextPollMs` after each response (clamped to a configurable floor, default 1000 ms) before issuing the next one. Events are yielded to the subscription's async iterator. The caller does not implement or manage the polling loop.
...
| Poll wait model | `events/poll` accepts an optional `waitMs`; a positive value makes it a long poll — the server holds the request until an event is available or `waitMs` elapses, and MUST respond immediately on either; omitted/`0` is an immediate poll; events are never delivered as notifications while the request is held open | Plain immediate request/response poll forces a trade-off between short intervals (fast reaction, mostly-empty requests) and long intervals (cheap, slow reaction). A bounded long poll collapses idle waiting into the held-open request while still bounding server-side resource use via `waitMs`, without adding subscription state — the wait is scoped to the single request. Making the long-poll behavior conditional on a positive `waitMs`, rather than universal, keeps a plain immediate poll available and keeps omission unambiguous across servers. Keeping delivery response-only (no SSE) preserves poll's simple request/response contract instead of blurring it with push. `nextPollMs` still governs the client's wait between polls, so servers keep the same dynamic-backoff lever. |
