...
| `-32014` | `EndpointVerificationFailed` | Callback endpoint was reachable but failed the verification handshake — did not echo the challenge (webhook mode only) |
...
These codes are drawn from `-32011..-32017` to avoid collision with base MCP error codes (e.g., `-32002 ResourceNotFound`).
...
| Unauthorized subscription, or continued delivery after access is revoked | Subscribe-time permission check (`-32012 Unauthorized`); periodic delivery-time re-verification; a `terminated` envelope ends the subscription when access is lost. *(Authorization)* |
...
| Server-side resource exhaustion (subscription flooding) | `-32013 TooManySubscriptions` limits; webhook subscriptions are TTL'd soft state that expire without refresh. *(Error Codes)* |
