...
**Delivery modes.** Three, advertised per event type; none is mandatory. **Poll** (`events/poll`) sends `{name, arguments, cursor, waitMs}`; a positive `waitMs` makes the call a bounded long poll — the server holds the request open and responds as soon as an event is available (possibly several at once), or once `waitMs` elapses with an empty `events[]` if none arrived. For a subscription consumed in poll mode, event occurrences MUST be returned in the `events/poll` response and MUST NOT additionally be delivered as notifications (e.g. SSE) on that request's connection. Omitting `waitMs` (or `0`) is an immediate, non-blocking poll. The response is `{events[], cursor, nextPollMs}`. **Push** (`events/stream`) is a long-lived request: events arrive as `notifications/events/event`, with a heartbeat carrying the current cursor during quiet periods. **Webhook** (`events/subscribe`) registers an `https` callback URL plus a client-supplied `whsec_` secret; the server POSTs each event there, signed per Standard Webhooks (`webhook-id/-timestamp/-signature` + `X-MCP-Subscription-Id`); subscriptions carry a TTL the client suggests (`ttlMs`) and the server grants (`refreshBefore`, SHOULD be ≤ the suggestion; `null` = no expiry), and the client refreshes before `refreshBefore`. The client SDK negotiates the mode and runs the loop; the model only sees arriving events.
...
The client SDK calls `events/poll` at the server-recommended interval, normally passing a positive `waitMs` so the call is a bounded long poll: the server holds the request open until either a new event becomes available or `waitMs` elapses, and MUST respond immediately once an event is available. Omitting `waitMs` (or passing `0`) requests an immediate, non-blocking poll instead. This is a protocol-level operation, NOT an LLM tool call.
...
sequenceDiagram
    ...
    loop
        SDK->>Server: events/poll {name, arguments, cursor, waitMs}
        Note over Server: holds request open until an event<br/>arrives or waitMs elapses
...
```jsonc
{
  ...
  "waitMs": 30000                  // optional, non-negative; how long the server may hold the request open waiting for an event. Omitted or 0 = immediate poll (no wait)
}
```
...
- `waitMs` is an optional non-negative integer specifying how long the client permits the server to hold the request while waiting for matching events. When omitted or set to `0`, the server responds immediately. When greater than `0`, the server MAY hold the request for up to the requested duration. The server MUST respond as soon as one or more matching events become available and MUST initiate a response no later than the requested duration. The server MAY impose a lower implementation-defined maximum. Absent such a constraint, the server SHOULD NOT return an empty `events` array before `waitMs` elapses — accepting `waitMs` while routinely returning early empty results would behave like short polling despite advertising a long poll; a genuine implementation or transport constraint (e.g. a proxy or connection timeout) is a valid reason to return earlier.
- Poll delivery is response-only: for a subscription consumed in poll mode, event occurrences MUST be returned in the `events/poll` response and MUST NOT additionally be delivered as notifications (e.g. SSE `notifications/events/event`) on that request's connection. This does not restrict delivery to the same event type's other subscriptions consumed via push or webhook — it scopes only to the poll-mode subscription's own connection. Holding the request open only extends how long the server waits before answering it; the events themselves are returned exclusively in the `events/poll` response body. Servers that want notification-based delivery for a given subscription use push (`events/stream`) instead.
...
- `nextPollMs` allows the server to dynamically adjust polling frequency (e.g., back off when rate-limited upstream, speed up when activity is detected). Ignored when `hasMore` is `true`. Clients MUST wait `nextPollMs` before issuing the next `events/poll`, regardless of whether the prior response was triggered by an available event or by `waitMs` elapsing. Clients MAY apply a configurable minimum delay (default 1000 ms) after an immediate or prematurely returned empty poll, to guard against a misbehaving server inducing a tight loop; no minimum delay should be added after a normally completed bounded long poll where `nextPollMs` is `0`, since the wait already occurred server-side.
- Empty `events` array means nothing happened during the wait — this is the common case, returned either once `waitMs` elapses or earlier at the server's discretion (see `waitMs` above), and should be cheap.
...
**For poll mode:** The SDK calls the check function for the incoming `events/poll` request. If it reports no events and `waitMs` is greater than `0`, the SDK holds the request open and re-invokes the check function — on a short internal cadence, or immediately on `server.emit()` for that event name — until an event is found or `waitMs` elapses, then returns the result. When `waitMs` is omitted or `0`, the SDK returns immediately without waiting. The server remains stateless across requests: the wait is scoped to the lifetime of the single held-open request, not to persisted subscription state.
...
**Poll mode:** The SDK runs a background loop per subscription that calls `events/poll`, passing a `waitMs` for the server to hold the request open, and waits `nextPollMs` after each response before issuing the next one. It applies a configurable minimum delay (default 1000 ms) only after an immediate or prematurely returned empty poll, to guard against a tight loop — not after a normally completed bounded long poll where `nextPollMs` is `0`. Events are yielded to the subscription's async iterator. The caller does not implement or manage the polling loop.
...
| Poll wait model | `events/poll` accepts an optional `waitMs`; a positive value makes it a long poll — the server holds the request until an event is available or `waitMs` elapses, and MUST respond immediately on either; omitted/`0` is an immediate poll; for a subscription consumed in poll mode, its events are returned only in that request's response, never additionally as notifications on that connection | Plain immediate request/response poll forces a trade-off between short intervals (fast reaction, mostly-empty requests) and long intervals (cheap, slow reaction). A bounded long poll collapses idle waiting into the held-open request while still bounding server-side resource use via `waitMs`, without adding subscription state — the wait is scoped to the single request. Making the long-poll behavior conditional on a positive `waitMs`, rather than universal, keeps a plain immediate poll available and keeps omission unambiguous across servers. Keeping delivery response-only for that subscription (no SSE on its own connection) preserves poll's simple request/response contract instead of blurring it with push, without constraining how the server delivers other subscriptions. `nextPollMs` still governs the client's wait between polls, so servers keep the same dynamic-backoff lever. |
